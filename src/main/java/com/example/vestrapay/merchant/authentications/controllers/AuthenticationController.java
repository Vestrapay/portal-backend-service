package com.example.vestrapay.merchant.authentications.controllers;
...
public class AuthenticationController {
    ...
    @PostMapping("login")
    public Mono<ResponseEntity<Response<User>>> twoFactorFirstLogin(@RequestBody LoginDTO request){
        return authenticationService.twoFactorFirstLogin(request)
                .map(userResponse -> ResponseEntity.status(userResponse.getStatus()).body(userResponse));
    }

    @PostMapping("2fa-login")
    public Mono<ResponseEntity<Response<LoginResponseDTO>>> login(@RequestBody FirstLoginDTO request){
