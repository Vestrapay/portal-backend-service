package com.example.vestrapay.superadmin.payment.controller;
...
public class PaymentController {
    ...
    @GetMapping("view-all-routes")
    @PreAuthorize("hasAuthority('CREATE_ADMIN')")
    public Mono<ResponseEntity<Response<?>>>viewAllRoutes(){
        return paymentMethodInterface.viewAllRoutes()
                .map(response -> ResponseEntity.status(response.getStatus()).body(response));
    }
}
