package com.example.vestrapay.superadmin.payment.services;
...
import com.example.vestrapay.superadmin.payment.interfaces.IPaymentMethodInterface;
import com.example.vestrapay.superadmin.payment.model.PaymentMethods;
import com.example.vestrapay.superadmin.payment.model.PaymentProviders;
...
import com.example.vestrapay.superadmin.payment.repository.PaymentProviderRepository;
import com.example.vestrapay.superadmin.payment.repository.RoutingRuleRepository;
...
import static com.example.vestrapay.utils.Constants.FAILED;
import static com.example.vestrapay.utils.Constants.SUCCESSFUL;
...
public class PaymentService implements IPaymentMethodInterface {
    ...
    public Mono<Response<Object>> viewAllProvidersByPaymentMethod(String paymentMethod) {
        return paymentProviderRepository.findAll()
                .collectList()
                .flatMap(paymentProviders -> {
                    Set<PaymentProviders> collect = new HashSet<>();
                    paymentProviders.forEach(paymentProviders1 -> {
                        Set<String> methods = new HashSet<>();
                        paymentProviders1.getSupportedPaymentMethods().forEach(s -> {
                            String value = s.replace("}","").replace("{","");
                            methods.add(value);
                            if (methods.contains(paymentMethod)){
                                collect.add(paymentProviders1);
                            }
                        });
                    });
                    ...
                    return paymentMethodRepository.findAll().collectList().flatMap(paymentMethods -> {
                        ...
                        if (collect.contains(request.getPaymentMethod())){
                            return paymentProviderRepository.findByName(request.getProvider())
                                    .flatMap(paymentProviders -> {
                                        Set<String> methods = new HashSet<>();
                                        paymentProviders.getSupportedPaymentMethods().forEach(s -> {
                                            String value = s.replace("}","").replace("{","");
                                            methods.add(value);
                                        });
    ...
    @Override
    public Mono<Response<Object>> viewAllRoutes() {
        return routingRuleRepository.findAll()
                .collectList()
                .flatMap(routingRules -> {
                    return Mono.just(Response.builder()
                                    .status(HttpStatus.OK)
                                    .statusCode(HttpStatus.OK.value())
                                    .message(SUCCESSFUL)
                    ...
                    return Mono.just(Response.builder()
                                    .errors(List.of("Routes not found"))
                                    .message(SUCCESSFUL)
                                    .statusCode(HttpStatus.NOT_FOUND.value())
                                    .status(HttpStatus.NOT_FOUND)
                            .build());
